...
Zip file at #{file_path} inaccessible.
        ...
        begin
          ...
          nodelist.each do |key,value|            
            ...
            payload_to_zip(value,top_dir)
          end

          # get all zips in in the source, if it's a dir or not
          top_lvl_zip_paths = expand_dir(top_dir)

          # for each of these, extract to /tmp/
          top_lvl_zip_paths.each { |zip_path| extract_zip(zip_path, top_dir2) }

          # extract any zips in these zips
          recursively_extract_zips(top_dir2)

          # remove empty file paths from the tmp dir
          clean_dir(top_dir2)

          # parse the extracted files to yaml
          process_container_dir(top_dir2)
        ensure
          FileUtils.remove_entry top_dir
          FileUtils.remove_entry top_dir2
        end
      end

      private
      def payload_to_zip(node,dir)
        contents = node["payload"]
        file_path=File.join(dir,"#{node["hostname"]}")
        File.open(file_path, 'w+') do |f|
          f.write(contents)
        end
        system("LC_ALL=C tr -cd 0-9a-fA-F < #{file_path} | xxd -r -p > #{file_path}.zip")
      end
